...
@Query(value = " SELECT p FROM Product p " +
        "WHERE p.address.id = :addressId AND p.isClosed = false AND p.isMatched = false AND p.isDeleted = false AND p.name Like %:words%")
    Page<ProductAllDTO> searchAll(@Param("addressId")long addressId, @Param("words") String words, Pageable pageable);

    @Query(value = "select p " +
            "from Product p " +
            "where p.address.id = :addressId and p.isClosed = false and p.isMatched = false and p.isDeleted = false ")
    ...
    @Query(value = "select count(p) " +
            "from Product p " +
            "where p.user.id = :userId and p.isClosed = false and p.isDeleted = false and p.isMatched = false and p.isClosed = false")
